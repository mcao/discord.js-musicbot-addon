...
exports.start = (client, options) => {
  ...
  class Music {
    constructor(client, options) {
      ...
      this.play = {
        ...
        help:
          (options && options.play && options.play.help) ||
      ...
      this.help = {
        ...
        help:
          (options && options.help && options.help.help) ||
          "Help for commands.",
      ...
      this.pause = {
        ...
        help:
          (options && options.pause && options.pause.help) ||
      ...
      this.resume = {
        ...
        help:
          (options && options.resume && options.resume.help) ||
      ...
      this.leave = {
        ...
        help:
          (options && options.leave && options.leave.help) ||
      ...
      this.queue = {
        ...
        help:
          (options && options.queue && options.queue.help) ||
      ...
      this.np = {
        ...
        help:
          (options && options.np && options.np.help) ||
      ...
      this.loop = {
        ...
        help:
          (options && options.loop && options.loop.help) ||
      ...
      this.clearqueue = {
        ...
        help:
          (options && options.clear && options.clear.help) ||
      ...
      this.volume = {
        ...
        help:
          (options && options.volume && options.volume.help) ||
      ...
      this.remove = {
        ...
        usage:
          (options && options.remove && options.remove.usage) ||
      ...
      this.skip = {
        ...
        help:
          (options && options.skip && options.skip.help) ||
      ...
      this.cooldown = {
        disabled: Boolean(
          options && options.cooldown && options.cooldown.disabled
        ),
        timer: parseInt(
  ...
  client.on("message", msg => {
    ...
    const suffix = message
      ...
      .trim();
    const args = message
      .slice(musicbot.botPrefix.length + command.length)
      .trim()
      .split(/ +/g);

    if (
      message.startsWith(musicbot.botPrefix) &&
      !msg.author.bot &&
      msg.channel.type == "text"
    ) {
      if (musicbot.commands.has(command)) {
        if (
          musicbot.recentTalk.has(msg.author.id) &&
          !musicbot.cooldown.disabled
        )
  ...
  musicbot.helpFunction = (msg, suffix, args) => {
    ...
    if (!suffix) {
      if (msg.channel.permissionsFor(msg.guild.me).has("EMBED_LINKS")) {
        ...
        let max = musicbot.commandsArray.length;
        for (var i = 0; i < musicbot.commandsArray.length; i++) {
          if (!musicbot.commandsArray[i].disabled) {
            cmdmsg =
              cmdmsg +
              `\n• ${musicbot.commandsArray[i].name}: ${
                musicbot.commandsArray[i].help
  ...
  musicbot.resumeFunction = (msg, suffix, args) => {
    ...
    const dispatcher = voiceConnection.player.dispatcher;
    if (!dispatcher.paused)
      return msg.channel.send(musicbot.note("fail", `Music already playing.`));
  ...
  musicbot.leaveFunction = (msg, suffix) => {
    if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
      ...
      setTimeout(() => {
        let vc = client.voiceConnections.find(
          val => val.channel.guild.id == msg.guild.id
        );
  ...
  musicbot.npFunction = (msg, suffix, args) => {
    ...
    if (voiceConnection === null && queue.length > 0)
      return msg.channel.send(
        musicbot.note(
          "fail",
          "No music is being played, but an ongoing queue is avainable."
        )
      );
  ...
  musicbot.clearFunction = (msg, suffix, args) => {
    ...
    musicbot
      ...
      .then(res => {
        return msg.channel.send(musicbot.note("fail", "Queue cleared."));
        const voiceConnection = client.voiceConnections.find(
          val => val.channel.guild.id == msg.guild.id
        );
  ...
  musicbot.removeFunction = (msg, suffix, args) => {
    ...
    if (!musicbot.canSkip(msg.member, musicbot.queues.get(msg.guild.id)))
      return msg.channel.send(
        musicbot.note("fail", "You can't remove that as you didn't queue it.")
      );
              ...
              if (queue.songs.length > 0) {
                if (loop == "none" || loop == null) {
                  ...
                  musicbot
                    ...
                    .catch(() => {
                      console.error(
                        new Error("something went wrong moving the queue")
                      );
                    });
